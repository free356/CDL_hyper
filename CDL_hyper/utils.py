...
def evaluation(test_R,test_mask_R,Estimated_R,num_test_ratings):
    ...
    print(num_test_ratings)
    print("sum_test_mask_R")
    print(np.sum(test_mask_R))
    """
    ...
    m=30
    sum=0
    for j in range(0,len(test_R)):
        lar=heapq.nlargest(m,Estimated_R[j])
        count=0
        for i in range(0,m):
            pre_item_index=(Estimated_R[j].tolist().index(lar[i]))
            if test_R[j][pre_item_index]==1:
                count+=1        
        rating=np.sum(test_R[j])
        if rating==0:
            recall=0
        else:
            recall=count/rating
        sum+=recall
    recall_M=sum/(len(test_R))
    print(recall_M)
    return RMSE,MAE,ACC,AVG_loglikelihood,recall_M

def make_records(result_path,test_acc_list,test_rmse_list,test_mae_list,test_avg_loglike_list,recall_list,current_time,
                 args,model_name,data_name,train_ratio,hidden_neuron,random_seed,optimizer_method,lr):
    if not os.path.exists(result_path):
        os.makedirs(result_path)

    overview = result_path + 'overview.txt'
    ...
    with open(test_record, 'w') as g:
        ...
        g.write(str("recall_M:"))
        g.write('\t')
        for itr in range(len(recall_list)):
            g.write(str(recall_list[itr]))
            g.write('\t')
        g.write('\n')
