...
class CDL():
    def __init__(self,sess,hyper, num_user, num_item, num_voca, hidden_neuron,current_time,
    ...
    def prepare_model(self):
        ...
        self.model_input_R = tf.placeholder(dtype=tf.float32, shape=[self.num_user,None], name="model_input_R")#?这里不理解
        ...
        with tf.variable_scope("CDL_Variable",reuse=tf.AUTO_REUSE):
        ...
        #tf.gather类似于数组的索引，可以把向量中某些索引值提取出来，得到新的向量，适用于要提取的索引为不连续的情况。这个函数似乎只适合在一维的情况下使用。
        ...
        with tf.variable_scope("CD",reuse=tf.AUTO_REUSE):
            if self.optimizer_method == "Adam":
                optimizer1 = tf.train.AdamOptimizer(self.hyperparams.lr)
                optimizer2 = tf.train.AdamOptimizer(self.hyperparams.lr)
            elif self.optimizer_method == "Momentum":
                optimizer1 = tf.train.MomentumOptimizer(self.hyperparams.lr,0.9)
                optimizer2 = tf.train.MomentumOptimizer(self.hyperparams.lr,0.9)
                ...
                optimizer1 = tf.train.AdagradOptimizer(self.hyperparams.lr)
                optimizer2 = tf.train.AdagradOptimizer(self.hyperparams.lr)
    ...
    def run(self):
        ...
        while True:
            ...
            if no_improvement_counter >= 2:
                break
            #在全部测试集上获取loss
        Cost,RMSE, MAE, ACC, AVG_loglikelihood,recall_M=self.test_model(epochs)
        make_records(self.result_path,self.test_acc_list,self.test_rmse_list,self.test_mae_list,self.test_avg_loglike_list,self.recall_list,self.current_time,
        ...
        variable_save(self.result_path, self.model_name,self.train_var_list1, self.train_var_list2, self.Estimated_R, self.test_R,
                      self.test_mask_R)
        return Cost,RMSE, MAE, ACC, AVG_loglikelihood,recall_M
    ...
    def test_model(self, itr):
        ...
        RMSE, MAE, ACC, AVG_loglikelihood,recall_M = evaluation(self.test_R, self.test_mask_R, self.Estimated_R,
        ...
        self.recall_list.append(recall_M)
        ...
        if itr % self.display_step == 0:
            ...
            print("RMSE = {:.4f}".format(RMSE), "MAE = {:.4f}".format(MAE), "ACC = {:.10f}".format(ACC),
                  "AVG Loglike = {:.4f}".format(AVG_loglikelihood),"RECALL = {:.4f}".format(recall_M))
            print("=" * 100)
        ...
        if (itr > 100) and (self.patience >= self.total_patience):
            ...
            self.recall_list.append(self.recall_list[self.min_epoch])
